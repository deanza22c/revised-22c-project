...
private:
	...
	int arrayLength;
	int itemCount = 0;
	LinkedList<Pokemon*> pLink; //ONLY FOR LINKED LIST COLLISION
	...
	*/
	...
	{
		return pLink.searchAnywhere(data);
	}
...
public:
	...
	*/
	...
	{
		Node<Pokemon*>* temp = pLink.obtainDataAnywhere(index);
	...
	*/
	void hashDisplayAll()
	{
		...
		int linkedSize = pLink.getCurrentSize();
		...
		if (pArray[index] != nullptr)
		{
			...
			*/
			...
			else
			{
				...
				isRemoved = pLink.removeAnywhere(index);
				return isRemoved;
			}
		}
		else
	...
	*/
	...
	{
		if (index >= 0 && index < arrayLength && pArray[index] != nullptr)
		{
			delete pArray[index];
			pArray[index] = nullptr;
			itemCount--;
